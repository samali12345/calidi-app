...
exports.getProducts = async (req, res) => {
  try {
    ...
    const IMAGE_BASE = `${process.env.BACKEND_PUBLIC_URL || 'https://calidi-app-production.up.railway.app'}/api/products/image`;

    const result = products.map(p => ({
      ...p.toObject(),
      image: p.image_id ? `${IMAGE_BASE}/${p.p_id}` : p.img
    }));

    res.json({ products: result, total, page, pages: Math.ceil(total / limit) });
  } catch (err) {
